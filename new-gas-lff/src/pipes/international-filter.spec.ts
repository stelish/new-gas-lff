...
describe('Pipe: InternationalFilterPipe',() => {
  ...
  it('should filter out and leave on domestic items', async(() => {
    ...
    const filtered = new InternationalFilterPipe().transform(mockItems);
    expect(filtered).toEqual(jasmine.any(Array));
    expect(filtered.length).toBe(1);
    expect(filtered[0].destinationName).toBe("Niue");
    expect(filtered[0].destinationIataCode).toBe("IUE");
  }));

  it('should return null if no items provided', async(() => {
    const filtered = new InternationalFilterPipe().transform(null);
